...
func TestLoadCatalogPhase1EmptyBody(t *testing.T) {
	work := t.TempDir()
	writeSkillDir(t, filepath.Join(work, ".johnnycode", "skills"), "demo",
...
func TestCatalogGetFullHotReload(t *testing.T) {
	work := t.TempDir()
	dir := writeSkillDir(t, filepath.Join(work, ".johnnycode", "skills"), "hot",
...
func TestLoadCatalogProjectOverridesBuiltin(t *testing.T) {
	work := t.TempDir()
	writeSkillDir(t, filepath.Join(work, ".johnnycode", "skills"), "commit",
