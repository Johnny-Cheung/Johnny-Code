...
func (f *fetcher) listContents(src *SkillSource, subpath string) ([]contentEntry, error) {
	...
	req.Header.Set("User-Agent", "johnnycode-install-skill")
...
// is expected to be the user-global skills tier (~/.johnnycode/skills/) so
...
// UserSkillsRoot returns ~/.johnnycode/skills, creating the parent if needed
...
func UserSkillsRoot() (string, error) {
	...
	root := filepath.Join(home, ".johnnycode", "skills")
