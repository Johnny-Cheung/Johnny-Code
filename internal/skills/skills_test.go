...
func TestLoadJohnnycodeSkills(t *testing.T) {
	wd, _ := os.Getwd()
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .johnnycode/skills directory found")
	}
...
func TestLoadSkillsMergesPriority(t *testing.T) {
	work := t.TempDir()
	johnnycodeDir := filepath.Join(work, ".johnnycode", "skills", "shared")
	if err := os.MkdirAll(johnnycodeDir, 0o755); err != nil {
		t.Fatal(err)
	}
	os.WriteFile(filepath.Join(johnnycodeDir, "SKILL.md"), []byte(`---
name: shared
description: project skill from .johnnycode
---
johnnycode body`), 0o644)
	...
	if !strings.Contains(got.PromptBody, "johnnycode body") {
		t.Errorf("expected johnnycode body; got body=%q", got.PromptBody)
	}
}

func TestLoadSkillsAllowedTools(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".johnnycode", "skills", "limited")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: limited
...
func TestLoadSkillsContextFork(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".johnnycode", "skills", "forky")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: forky
