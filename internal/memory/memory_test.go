...
func TestGetAutoMemPath(t *testing.T) {
	t.Setenv("JOHNNYCODE_REMOTE_MEMORY_DIR", "")
	root := "/tmp/foo/project"
	path := GetAutoMemPath(root)
	if !strings.HasSuffix(path, "/.johnnycode/memory/") {
		t.Errorf("expected suffix /.johnnycode/memory/, got: %s", path)
	}
...
func TestGetAutoMemPathRespectsOverride(t *testing.T) {
	t.Setenv("JOHNNYCODE_REMOTE_MEMORY_DIR", "/custom/memdir")
...
func TestIsAutoMemPath(t *testing.T) {
	...
	cases := map[string]bool{
		...
		dir + "foo.md":               true,
		dir + "sub/foo.md":           true,
		"/tmp/p/.johnnycode/memoryx":    false,
