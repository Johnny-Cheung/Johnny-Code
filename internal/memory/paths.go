...
// project root. Shape: <projectRoot>/.johnnycode/memory/
//
...
// `…/memoryxyz`.
//
// Johnny Code colocates memory with other project-local state under .johnnycode/
// so records show up in the IDE and editors can open them directly.
//
// Resolution order:
//  1. JOHNNYCODE_REMOTE_MEMORY_DIR env var — used as-is (escape hatch for
//     CI/container scenarios where memory should live elsewhere)
//  2. <projectRoot>/.johnnycode/memory
func GetAutoMemPath(projectRoot string) string {
	if override := os.Getenv("JOHNNYCODE_REMOTE_MEMORY_DIR"); override != "" {
	...
	return filepath.Join(abs, ".johnnycode", "memory") + string(filepath.Separator)
}
...
// ~/.johnnycode/memory/. Used for type=user / type=feedback memories that
...
func GetUserAutoMemPath() string {
	...
	return filepath.Join(home, ".johnnycode", "memory") + string(filepath.Separator)
}

// GetUserAutoMemEntrypoint returns the path to ~/.johnnycode/memory/MEMORY.md.
