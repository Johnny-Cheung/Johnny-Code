...
// running the suite doesn't litter the repo with .johnnycode/teams/
// directories.
func TestMain(m *testing.M) {
	tmp, err := os.MkdirTemp("", "johnnycode-teams-test-")
	if err != nil {
		panic(err)
	}
	_ = os.Setenv("JOHNNYCODE_TEAMS_DIR", filepath.Join(tmp, "teams"))
	code := m.Run()
	_ = os.RemoveAll(tmp)
	os.Exit(code)
}
