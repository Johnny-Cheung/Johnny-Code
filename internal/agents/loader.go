...
// locally that's a JOHNNYCODE_VERIFICATION_AGENT env var.
func getBuiltinSpecs() map[string]SubAgentSpec {
	...
	if os.Getenv("JOHNNYCODE_VERIFICATION_AGENT") == "true" {
...
func (l *AgentLoader) LoadAll() error {
	...
	if home != "" {
		l.loadDir(filepath.Join(home, ".johnnycode", "agents"), "user")
	}

	if l.workDir != "" {
		l.loadDir(filepath.Join(l.workDir, ".johnnycode", "agents"), "project")
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			...
			if l.ErrorWriter != nil {
				fmt.Fprintf(l.ErrorWriter, "[johnnycode] agent definition skipped — %s\n", msg)
			}
			continue
		}
		def.Source = source
