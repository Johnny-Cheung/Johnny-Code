...
import (
	...
	"johnnycode/internal/agent"
	"johnnycode/internal/agents"
	"johnnycode/internal/commands"
	"johnnycode/internal/compact"
	"johnnycode/internal/config"
	"johnnycode/internal/conversation"
	"johnnycode/internal/filehistory"
	"johnnycode/internal/history"
	"johnnycode/internal/hooks"
	"johnnycode/internal/llm"
	"johnnycode/internal/mcp"
	"johnnycode/internal/memory"
	"johnnycode/internal/memory/extractor"
	"johnnycode/internal/permissions"
	"johnnycode/internal/planfile"
	"johnnycode/internal/prompt"
	"johnnycode/internal/session"
	"johnnycode/internal/skills"
	"johnnycode/internal/teams"
	"johnnycode/internal/todo"
	"johnnycode/internal/tools"
	"johnnycode/internal/worktree"
	...
	case initSingleProviderMsg:
		...
		ag.Checker = permissions.NewChecker(
			...
			&permissions.RuleEngine{
				LocalPath: filepath.Join(wd, ".johnnycode", "permissions.local.yaml"),
			},
...
func (m *Model) loadSkillsAndBuildPrompt(wd string) string {
	...
	skillsDir := filepath.Join(wd, ".johnnycode", "skills")
	skillSection := ""
	if catalog != nil {
		metas := catalog.List()
		if len(metas) > 0 {
...
func scanFiles(prefix string, limit int) []string {
	...
	skipDirs := map[string]bool{
		...
		"__pycache__": true, ".johnnycode": true, "vendor": true,
	}
...
func (m Model) renderPlanApprovalDialog() string {
	...
	header := lipgloss.NewStyle().Foreground(brandPurple).Bold(true).Render(
		" Johnny Code has written up a plan and is ready to execute. Would you like to proceed?",
	)
	...
	options := []string{
		...
		"Tell Johnny Code what to change",
	}
...
func (m Model) handleAgentEvent(ev agent.AgentEvent) (tea.Model, tea.Cmd) {
	...
	case agent.ToolResultEvent:
		...
		if emitted != nil {
			// Flush pending assistant text first so order matches Johnny Code's
...
func (m Model) renderBanner() string {
	cat := bannerStyle.Render(` /\_/\    `) + bannerDimStyle.Render("Johnny Code v0.1.0") + "\n" +
