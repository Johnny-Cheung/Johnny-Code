...
func TestLoadUserCommandsMergesAndOverrides(t *testing.T) {
	work := t.TempDir()
	mewProj := filepath.Join(work, ".johnnycode", "commands")
	mustWrite(t, filepath.Join(mewProj, "shared.md"), `from johnnycode`)
	...
	if !strings.Contains(names["shared"], "from johnnycode") {
		t.Errorf("expected 'from johnnycode'; got %q", names["shared"])
	}
