...
func TestLiveSkillCreatorOutputPath(t *testing.T) {
	...
	// Use a temp dir to simulate a project with .johnnycode/skills/
	workDir := t.TempDir()
	testSkillsDir := filepath.Join(workDir, ".johnnycode", "skills")
	...
	if _, err := os.Stat(expectedFile); os.IsNotExist(err) {
		...
		for _, f := range allFiles {
			if strings.Contains(f, "SKILL.md") && !strings.Contains(f, ".johnnycode/skills/") {
				t.Errorf("SKILL.md created at WRONG location: %s (should be under .johnnycode/skills/)", f)
				wrongLocation = true
			}
		}
		if !wrongLocation {
...
func TestLiveSimpleChat(t *testing.T) {
	...
	text, _ := liveRound(t, ag, conv, "Reply with exactly: JOHNNYCODE_OK")

	if !strings.Contains(text, "JOHNNYCODE_OK") {
		t.Errorf("expected JOHNNYCODE_OK in response, got: %s", truncate(text, 200))
	}
}
