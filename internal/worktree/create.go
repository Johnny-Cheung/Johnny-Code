...
// WorktreesDir is where all Johnny Code-managed worktrees live: a single directory inside the repo root
// that's already in .gitignore.
func WorktreesDir(repoRoot string) string {
	return filepath.Join(repoRoot, ".johnnycode", "worktrees")
}
...
// <repoRoot>/.johnnycode/worktrees/, or resumes it if it already exists.
//
